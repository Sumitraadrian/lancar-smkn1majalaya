...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
       ...
       // Hitung lebar teks
        ...
        $pdf->Cell(40, 8, 'Jurusan', 1, 0, 'C');
        $pdf->Cell(20, 8, 'Kelas', 1, 0, 'C');
        $pdf->Cell(50, 8, 'Keterangan', 1, 1, 'C');

        // Isi Tabel
        $pdf->Cell(10, 8, '1', 1, 0, 'C'); // No
        $pdf->Cell(40, 8, htmlspecialchars($pengajuan['nama_lengkap']), 1, 0, 'C'); // Nama
        $pdf->Cell(30, 8, htmlspecialchars($pengajuan['nis']), 1, 0, 'C'); // NIS
        $pdf->Cell(40, 8, htmlspecialchars($pengajuan['jurusan']), 1, 0, 'C'); // Jurusan
        $pdf->Cell(20, 8, htmlspecialchars($pengajuan['kelas']), 1, 0, 'C'); // Kelas
        $pdf->Cell(50, 8, htmlspecialchars($pengajuan['alasan']), 1, 1, 'C'); // Keterangan
        ...
        // Output PDF
        $pdf->Output('I', 'surat_dispensasi.pdf'); // 'I' untuk tampilkan langsung di browser
        exit();
    } else {
